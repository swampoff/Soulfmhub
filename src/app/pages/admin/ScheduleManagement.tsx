...
export function ScheduleManagement() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Guard: if the schedule response is an error or gateway 401, do NOT clear the list
      if (schedRes.error || schedRes.code === 401 || schedRes.message === 'Invalid JWT') {
        const errMsg = schedRes.error || schedRes.message || 'Unknown error';
        console.error('[Schedule] ❌ Server error loading schedules:', errMsg, schedRes);
        toast.error(`Schedule load error: ${errMsg}`);
  ...
  const handleDropPlaylist = async (playlist: Playlist, day: number, hour: number) => {
    ...
    try {
      ...
      // Detect Supabase gateway 401 (expired JWT) — response is {code: 401, message: "Invalid JWT"}
      if (res.code === 401 || res.message === 'Invalid JWT') {
        console.error('[Schedule] ❌ Gateway auth error (expired session?):', res);
        toast.error('Ошибка авторизации — попробуйте перезайти в админку');
        return;
      }
